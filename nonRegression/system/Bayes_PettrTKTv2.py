...
from switch_controllerTKTv2 import switch_ctrl
import numpy as np
...
class tester:
	...
	def test_run(self, x):
		...
		self.example = switch_ctrl(sys = dynsys(x = np.array([[2.5,0.5,0,0]]).transpose()), 
			# v1:1.2 -> v2:1
			params = 1*np.array([[1,0,1,0],[0,1,0,1]]))
...
data=pd.read_csv("/home/jeff/Desktop/Test/lowLevelTesting/temp/v2gross.csv", delimiter = ';')
data.columns = [0, 1, 2, 3, 4] 
	# sorting data frame 
data=data.sort_values(by=[3,1,2], ascending=[True,True,True])
data.to_csv("/home/jeff/Desktop/Test/lowLevelTesting/logs/LoLev_v2_"+str(a)+"_"+str(b)+"_"+str(t)+".csv", index = False, header=False, sep = ';')
# down to here 